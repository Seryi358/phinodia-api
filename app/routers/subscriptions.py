...
from app.services.subscription_plans import PLANS_BY_CODE
...
async def status(subscription_id: str):
    """Estado de la suscripción + del reto 3DS de su fuente (polling del frontend)."""
    try:
        return await subs.get_status(subscription_id)
    except HTTPException:
        raise
    except Exception as e:
        _raise_http(e)
...
async def activate(subscription_id: str):
    ...
    try:
        return await subs.activate_subscription(subscription_id)
    except HTTPException:
        raise
    except Exception as e:
        _raise_http(e)
...
@router.get("")
async def mine(email: EmailStr = Query(...)):
    """Lista las suscripciones asociadas a un email (proyección segura)."""
    try:
